...
MATH_RE = re.compile(r"\$\$.*?\$\$|(?<!\\)\$(?!\$).*?(?<!\\)\$|\\\[.*?\\\]|\\\(.*?\\\)|\\begin\{(?:equation\*?|align\*?|gather\*?)\}.*?\\end\{(?:equation\*?|align\*?|gather\*?)\}", re.DOTALL)
...
def items(pattern: re.Pattern[str], text: str, *, split_commas: bool = False) -> Counter[str]:
    values: list[str] = []
    for match in pattern.findall(text):
        ...
        if split_commas:
            ...
            values.append(re.sub(r"\s+", " ", match.strip()))
...
def audit(original_path: Path, revised_path: Path, *, strict: bool = False) -> dict[str, object]:
    ...
    delta("citations", items(CITE_RE, original, split_commas=True), items(CITE_RE, revised, split_commas=True), errors)
    ...
    delta("DOIs", items(DOI_RE, original), items(DOI_RE, revised), errors)
    delta("URLs", items(URL_RE, original), items(URL_RE, revised), errors)
